...
// ------------------------------------------------ End Constructors ----------------------------------------------------------------------
        ...
        public bool moveAndDodge(Vector2 posZ, float speed, Sprite3 obstacle, float pushBackDist) 
        {
            bool retv = false;
            Vector2 spritePos = getPos();
            Vector2 baseDelta = posZ - spritePos;
            Vector2 moveDelta = baseDelta / baseDelta.Length();
            moveDelta = moveDelta * speed;
            Vector2 pushBack = new Vector2(0, 0);

            float pushBackDistance = pushBackDist + obstacle.boundingSphereRadius + boundingSphereRadius;
            Vector2 tempPos = obstacle.getPos();
            if ((tempPos - spritePos).Length() < pushBackDistance)
            {
                Vector2 pb = tempPos - spritePos;
                float dist = pb.Length();
                float amt = (pushBackDistance - dist) / pushBackDistance;
                amt = amt * 5f;// amount of push back (checked with debugger)
                pb = pb / dist;
                pushBack = pb * (-3);
            }
            Vector2 newB1Pos = spritePos + moveDelta + pushBack;
            //if ((newB1Pos - posZ).Length() < 0.8) { return true; }
            if ((newB1Pos - posZ).Length() < speed/2) { return true; }
            setPos(newB1Pos);
            //alignDisplayAngle();
            
            return retv;
        }
        public bool bossMoveAndDodge(Vector2 posZ, float speed, SpriteList sl, float pushBackDist)
        {
            bool retv = false;
            Vector2 spritePos = getPos();
            Vector2 baseDelta = posZ - spritePos;
            ...
            for (int i = 0; i <= sl.highUsed; i++)
            {
                Sprite3 ss = sl.getSprite(i);
                if (ss == null) continue;
                if (!ss.getVisible()) continue;
                if (!ss.active) continue;

                float pushBackDistance = pushBackDist + ss.boundingSphereRadius + boundingSphereRadius;
                Vector2 tempPos = ss.getPos();
                if ((tempPos - spritePos).Length() < pushBackDistance)
                {
                    Vector2 pb = tempPos - spritePos;
                    float dist = pb.Length();
        ...
        public int bombCollision(SpriteList sl, Sprite3 ground, Sprite3 main, HealthBar hb)
        {
            Vector2[] tempPos = new Vector2[3] {new Vector2(-100, 450), new Vector2(550, -100), new Vector2(1300, 450) };
            Rectangle bombBB = getBoundingBoxAA();
            int rc = sl.collisionWithRect(bombBB);
            bool gc = collision(ground);
            bool mc = collision(main);
        ...
        public bool moveTo1(Vector2 posZ, float speed, bool setDisplayAngle, float gravity)
        {
            bool retv = false;
            oldPos.X = pos.X;
            oldPos.Y = pos.Y;
            ...
            if (dist < speed) { s = dist; retv = true; }
            float moveAngle = angleTo(posZ);
            Vector2 temp = Util.moveByAngleDist(new Vector2(pos.X, pos.Y + gravity), moveAngle, s);
            pos.X = temp.X;
            pos.Y = temp.Y;
        ...
        public void setTickBetweenFrame(int tick)
        {
            ticksBetweenFrames = tick;
        }
